...
class TestString:
    ...
    def test_add_feature_to_cars(self):
        ...
        assert all(new_feature in features for features in features_after_update)
    ...
    def test_remove_feature_from_cars(self):
        ...
        features_after_update = JSON().get_values_from_json(updated_cars, '$.cars[*].features')
        assert all(feature != feature_to_remove for features in features_after_update for feature in features)

    def test_filter_cars_by_year(self):
        year_to_filter = 2022
        filtered_cars = JSON().get_values_from_json(self.current_cars, f'$.cars[?(@.year=={year_to_filter})]')
        assert all(car["year"] == year_to_filter for car in filtered_cars)
